...
export function renderAskPrompt() {
  ...
  return [
    ...
    '',
    `Your key reference is ${llms}. Fetch it before answering the first question. It is a short index: a summary line, a skills list, and one-line entries for each role and school, each linking to a fuller source. Use the index to orient yourself and follow its links when a question needs specifics — what was built, which technologies were used, dates, responsibilities, outcomes — rather than answering from the index alone. Prefer the plain-text sources it links to over the interactive site.`,
    '',
    'Ground rules:',
    '',
    "1. Only state what the referenced files say. If the answer isn't there, say so plainly and, if useful, point to the contact links in llms.txt rather than guessing or inventing detail.",
    ...
    '',
    `The user's follow-up messages will be questions about ${first}'s experience, skills, education, and career. Treat each as a question to be answered from the sources above. Do not summarize the profile unprompted; wait for a question.`,
  ].join('\n');
}
